...
Every task pod runs a Leoflow **agent** ([ADR 0004](adr/0004-thin-agent.md)) that
...
is a **prerequisite for [warm worker pools](warm-pools.md)**.

!!! info "Pro / Kubernetes only"
...
[Warm worker pools → How to enable](warm-pools.md#how-to-enable).
...
[ADR 0055](adr/0055-secret-scoping-and-token-liveness.md). They narrow different
...
- [Warm worker pools](warm-pools.md) — the feature that requires the exchange transport.
- [Variables & Connections](variables-connections.md) — what the task-scoped token resolves.
- [ADR 0055 — Secret scoping and token liveness](adr/0055-secret-scoping-and-token-liveness.md).
- [ADR 0004 — The thin agent](adr/0004-thin-agent.md).
