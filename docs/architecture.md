...
Postgres ([ADR 0049](adr/0049-split-api-and-scheduler-roles.md)); `role=all`
collapses them into one process for Lite. See
[Operating modes](operating-modes.md#deployment-topology-role-split) for the
deployment topology.
...
**Split roles — API and scheduler ([ADR 0049](adr/0049-split-api-and-scheduler-roles.md)).**
...
([ADR 0008](adr/0008-jwt-auth.md)). For human login it also supports
**OIDC/SSO** ([ADR 0057](adr/0057-oidc-sso.md)): an Authorization Code + PKCE
...
cert-manager; see [Pro TLS](pro-tls.md). (`internal/agent/dial.go` builds the
...
([ADR 0056](adr/0056-task-log-object-sink.md)) for shared multi-team clusters,
...
[exchange flow](agent-credential-transport.md#exchange-projected-sa-token-tokenreview),
and the pod-scoped-vs-attempt-scoped split that makes warm-pool reuse safe is the
[two-token model](agent-credential-transport.md#the-two-token-model) (also reused
by link from [Warm worker pools](warm-pools.md)).

## Authentication: OIDC/SSO login flow

With OIDC configured ([ADR 0057](adr/0057-oidc-sso.md)), a browser logging in
