...
| `LEOFLOW_EXECUTION_WARM_POOLS_ENABLED` | `false` | Pro | Master switch for warm worker pools — N:1 pod reuse across attempts of one DAG version ([ADR 0058](adr/0058-warm-worker-pools.md)). Off ⇒ dedicated pod-per-task, byte-for-byte today's behavior. On requires `LEOFLOW_AUTH_AGENT_TOKEN_TRANSPORT=exchange` **and** `LEOFLOW_AUTH_SECRET_LIVENESS_MODE=enforce` (validated fail-closed at boot). Ignored by Lite. See [Warm worker pools](warm-pools.md). |
| `LEOFLOW_EXECUTION_MIN_IDLE_WORKERS` | `0` | Pro | Warm pods kept ready **per DAG version**. `0` = scale-to-zero. Operator floor when a DAG declares no warmth; clamped to `max_pool_size`. |
| `LEOFLOW_EXECUTION_MAX_POOL_SIZE` | `8` | Pro | Cap on warm workers a **single DAG version** may hold. Must be ≥ 1 when warm pools are on. |
| `LEOFLOW_EXECUTION_MAX_ATTEMPTS_PER_WORKER` | `50` | Pro | Attempts a warm worker serves before it drains + recycles (bounds stale-image / leaked-credential exposure). Must be ≥ 1 when warm pools are on. |
| `LEOFLOW_EXECUTION_MAX_WORKER_LIFETIME` | `1h` | Pro | Duration ceiling on a warm worker before it drains + recycles, independent of attempt count. Must be > 0 when warm pools are on. |
| `LEOFLOW_EXECUTION_WORKER_IDLE_TTL` | `5m` | Pro | Duration an idle warm worker is kept before recycle. Must be > 0 when warm pools are on. |
| `LEOFLOW_EXECUTION_MAX_WARM_PODS_PER_TENANT` | `100` | Pro | Cap on warm pods a **single tenant** may hold across all its DAG versions (M4). Reserve-then-ration — promised idle floors are always honored; enforced only by refusing to create new pods, never by deleting a busy worker. Must be ≥ 1 when warm pools are on. |
