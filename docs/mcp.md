# MCP server (`leoflow-mcp`)

`leoflow-mcp` is Leoflow's **Model Context Protocol** server ([ADR 0050](adr/0050-mcp-server.md)):
...
the Airflow-compatible `/api/v2` (via the typed [`pkg/client`](go-api.md)), carrying
...
[install](installation.md), or `go build ./cmd/leoflow-mcp`). It has two transports.
...
!!! note "One process, one control plane"
...
- [ADR 0050 — Model Context Protocol server](adr/0050-mcp-server.md): the full design, the security posture, and the untrusted-content threat model.
- [Go packages → `pkg/client`](go-api.md): the typed `/api/v2` client the MCP is built on.
