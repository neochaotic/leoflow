...
!!! info "Pro · off by default"
    ...
    ([ADR 0002](adr/0002-pod-per-task.md)), byte-for-byte unchanged. Lite ignores
...
([ADR 0058](adr/0058-warm-worker-pools.md)). The reuse is safe because a warm pod
...
- **DAGs that use the [staging volume](staging-volume.md).** A pod's volumes are
...
specific image + dependency closure ([ADR 0003](adr/0003-dag-as-image.md)); keying
...
enforcement from [ADR 0055](adr/0055-secret-scoping-and-token-liveness.md). The
...
[Agent credential transport](agent-credential-transport.md) for the full envvar-vs-
...
[Agent credential transport → RBAC](agent-credential-transport.md#rbac-the-tokenreviews-grant).
...
!!! warning "Enable on a shared production cluster is a reliability decision"
...
[Agent credential transport](agent-credential-transport.md#the-two-token-model)
page (with the [two-token diagram](agent-credential-transport.md#the-two-token-model)).
...
on the [Agent credential transport](agent-credential-transport.md) page.
...
[Agent credential transport](agent-credential-transport.md) page and in the
[Configuration reference](configuration.md#server-environment-leoflow_).

## See also

- [Agent credential transport](agent-credential-transport.md) — the envvar-vs-exchange
  transport, the two-token model, and the `tokenreviews` RBAC that warm pools require.
- [Architecture](architecture.md) — where warm pools and the exchange transport sit
  in the control plane.
- [ADR 0058 — Warm worker pools](adr/0058-warm-worker-pools.md) — the full decision
  record.
- [ADR 0055 — Secret scoping and token liveness](adr/0055-secret-scoping-and-token-liveness.md) —
  the security prerequisite this feature depends on.
- [Configuration reference](configuration.md) — every `LEOFLOW_*` server variable.
