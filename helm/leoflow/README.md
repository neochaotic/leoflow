...
| deployment.preStopSleepSeconds | int | `5` | Seconds the control-plane container sleeps in its `lifecycle.preStop` hook before the process is signalled. Endpoint removal is asynchronous: a terminating replica stays in kube-proxy's and every client's endpoint set for a propagation window, so task pods keep opening NEW agent log streams against a control plane that is already shutting down — and each one creates an empty log object for an attempt whose lines then have nowhere to land. This sleep spends the propagation window before SIGTERM, so those streams land on a replica that will still be alive to flush them. It uses the native `sleep` hook action (Kubernetes 1.29+) rather than `exec`, because the image is distroless and has no shell. It runs INSIDE `terminationGracePeriodSeconds` and ahead of the drain, so it adds to the shutdown budget — count it when sizing the grace. Set `0` to omit the `lifecycle` block entirely; single-replica installs that care more about `Recreate` upgrade downtime than about a few empty objects are the case for that. |
