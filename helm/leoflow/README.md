...
| taskNetworkPolicy.allowMetadataEgress | list | `[]` | Host CIDRs within the always-blocked cloud-metadata range to RE-ALLOW, scoped to a single host each (a `/32`, never the whole `169.254.0.0/16`). Needed for keyless external-secrets auth that reaches a metadata/link-local endpoint: GKE Workload Identity (`169.254.169.254/32`) and EKS Pod Identity (`169.254.170.23/32`). AWS IRSA needs nothing here (it uses the public STS endpoint). Empty keeps the full metadata block. |
