...
| taskPodSecurity.allowRoot | bool | `false` | Let task pods run as root. Leave `false` unless your task images genuinely require it (a legacy base, or a task that installs packages at run time). Turning it on drops `runAsNonRoot`, which makes the pod inadmissible under a `restricted` namespace — that rejection is the intended signal. The rest of the hardening (no privilege escalation, all capabilities dropped, RuntimeDefault seccomp) stays on regardless, since none of it depends on the UID. |
