...
| deployment.strategy | string | `""` | Update strategy: `RollingUpdate`, `Recreate`, or `""` (auto). Auto picks `Recreate` when `logs.persistence` is a ReadWriteOnce PVC — a RollingUpdate surges a second pod that Multi-Attach-deadlocks on the RWO volume and the rollout never converges (#868/#282) — and `RollingUpdate` otherwise (RWX or ephemeral emptyDir). Set explicitly to override. |
