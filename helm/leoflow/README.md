...
| taskPodSecurity.runAsNonRoot | bool | `false` | Refuse to start a task container whose image runs as UID 0. This completes Pod Security Admission's `restricted` set, so a `restricted` namespace will admit your task pods. Off by default because the images Leoflow ships cannot satisfy it yet: the `examples/` images run as root, and the runtime base declares `USER leoflow` — a name the kubelet cannot resolve to a UID, so it rejects the container even though the user is not root. Turn it on once your own task images carry numeric non-root UIDs, and provide a writable path for anything using the staging volume. |
