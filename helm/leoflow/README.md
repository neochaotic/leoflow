...
| probes | object | `{"liveness":{"failureThreshold":3,"initialDelaySeconds":10,"periodSeconds":20,"timeoutSeconds":5},"readiness":{"failureThreshold":3,"initialDelaySeconds":5,"periodSeconds":10,"timeoutSeconds":3},"startup":{"enabled":true,"failureThreshold":24,"periodSeconds":5}}` | Liveness/readiness probes for the leoflow-server container. Exposed so an operator can loosen the timeout under load: a busy scheduler can miss a 1s `/healthz` during a task-pod burst and be kubelet-killed mid-run, which cascades in-flight tasks to agent_lost (#860). Defaults are deliberately forgiving (5s liveness / 3s readiness timeout, 3 failures) rather than Kubernetes' 1s/3. `readiness.timeoutSeconds` has a **floor of 3** and the render fails below it: /readyz bounds its own work at 2s so it answers before the kubelet gives up, and a shorter kubelet timeout inverts that and you get a bare timeout instead of a 503 naming the failing dependency (#1041). To react faster, lower `readiness.periodSeconds` or `failureThreshold` instead. |
| probes.startup | object | `{"enabled":true,"failureThreshold":24,"periodSeconds":5}` | Startup gate. Kubernetes suppresses BOTH liveness and readiness until this succeeds, so a slow boot reads as "not ready yet" instead of being killed and restarted. Without it liveness points at the same listener the boot has not bound yet, and any boot-path dependency that is slow or stuck turns into a restart loop whose every cycle is recorded as `Completed exit=0`, because SIGTERM is handled cleanly (#1083). The budget below is `periodSeconds * failureThreshold` = 120s, deliberately larger than the liveness budget it replaces (10 + 3*20 = 70s); a boot that exceeds it still restarts, which is the honest semantic. Set `enabled: false` to go back to liveness policing the boot. |
