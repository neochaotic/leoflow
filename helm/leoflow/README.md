...
| resources | object | `{"limits":{"cpu":"1","memory":"512Mi"},"requests":{"cpu":"250m","memory":"256Mi"}}` | Resource requests + limits for the leoflow-server container. Defaults sized for a small Pro (50–500 DAGs); bump CPU+memory for larger deployments. The scheduler fans out task pods and answers liveness/readiness probes, so the request must leave enough CPU headroom that a task-pod burst doesn't CFS-throttle it into a probe timeout and a kubelet restart (#860). |
