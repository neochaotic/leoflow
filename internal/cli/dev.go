...
func runDev(cmd *cobra.Command, dir string, o devOptions) error {
	...
	return devWatchLoop(ctx, cmd, ws, makeReload(token), makeDeleteDag(token, uiURL), makeFetchRegistered(token, uiURL))
}
...
// up without restart.
func devWatchLoop(ctx context.Context, cmd *cobra.Command, ws *WorkspaceSpec, reload func() error, deleteDag func(dagID string) error, fetchRegistered func() (map[string]struct{}, error)) error {
	...
	// Boot-time reconcile (#404): deregister DAGs the control plane still has from
	// a reused metadata DB or a previous workspace but that are absent on disk now,
	// so Lite self-heals to match the workspace instead of showing un-removable
	// ghosts. Fail-safe — a fetch error skips cleanup rather than wiping anything.
	if deleteDag != nil && fetchRegistered != nil {
		registered, ferr := fetchRegistered()
		lastSeenDagIDs = reconcileStartup(registered, ferr, lastSeenDagIDs, deleteDag,
