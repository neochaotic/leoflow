...
> **Moved to 5.1:** `GET /ui/config` was implemented in Phase 5.1 (all 12
> spec-required ConfigResponse fields, `theme: null`, no `is_db_isolation_mode`).
> 5.3 only *tunes the values* of existing fields if a browser test demands it —
> it does not add the endpoint. The spec's ConfigResponse has **no**
> `assets_enabled`/`plugins_enabled` flags; `/ui/auth/menus` is the lever that
> hides sections, not config flags.

1. **Graceful stubs** (200, schema-valid empty per spec): /ui/dashboard/
...
2. **Write flows E2E** (already on /api/v2/*): verify the UI buttons trigger
   ...
   reflected on reload).
3. **docker-compose.yml** (full): postgres 16-alpine, redis 7-alpine,
...
4. **E2E suite** (test/e2e/full_flow_test.go, //go:build e2e): testcontainers-go
   ...
   `make e2e-full`.
5. **Screenshots** docs/screenshots/: 01-login, 02-dag-list, 03-grid-view,
   04-graph-view, 05-task-log, 06-triggered-run.
6. **README** Getting Started: real docker-compose command, 5-min demo with
   checkpoints, header screenshot.
7. **CHANGELOG** [0.1.0-rc1]: Phase 5 complete, compose getting-started, E2E suite.
8. **ADR 0007 (Airflow UI Compatibility) Revision History** — note the premise
...
9. **TDD:** each stub returns schema-valid empty + validates against spec; write
   aliases delegate to existing handlers. The full E2E IS the failing test —
   implement until green.

## Acceptance

stubs schema-valid empty; 501 detail on unsupported writes;
