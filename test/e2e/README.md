...
| `e2e.sh` | Pod-path E2E on k3d: build images, k3d import, agent-over-gRPC, real pod-per-task. Regression guards for the ADR 0040 features in the real pod path: generic operator/sensor execution, `ti.xcom_pull` chaining, `@task` run-context (`ds`), Admin Variable delivery, **multi-key XCom** (`LEOFLOW_PUSHES_PATH`), and **native bash Jinja templating** (`{{ ds }}`, #382) | minutes | `ci.yaml` job `e2e-operators` (k3d) on every PR + push |
