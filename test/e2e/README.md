...
| `chaos-runtime.sh` | **Runtime fault-injection (#231 Phase 2).** Kills the scheduler process mid-run (asserts `/monitor/health` flips the scheduler unhealthy via the advisory-lock reader, the in-flight run **resumes** to success, and **at-most-once** holds — no duplicate `work` pod), and force-deletes a running task pod (asserts the reaper moves the TI off `running` and leaves no orphan). Destructive; run inside the **Lima Linux VM**. `make chaos-runtime` | ~6 min | not in CI (exploratory/destructive) |
