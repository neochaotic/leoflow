...
run_experiment() {
  ...
  # A bounded exploratory stop, for inspecting a LIVE control plane instead of
  # discovering what is left one paid cluster at a time.
  #
  # BOUNDED, and the bound is the point: an unbounded pause is a forgotten
  # cluster, which is the one outcome this directory exists to prevent. The node
  # TTL stays underneath it as the backstop, and the pause is far shorter.
  if [ -n "${WP_PAUSE_AFTER_DEPLOY:-}" ]; then
    ...
    exp_log "  api:     http://127.0.0.1:$api_port (port-forward live)"
