...
- The **Airflow 3.2.x UI** (grid, graph, dashboard) — unmodified.
- The **vocabulary**: DAGs, tasks, runs, XCom, trigger rules.
- The **TaskFlow** authoring you already know.

## What you gain

- A **real dev loop** (`leoflow lite` — isolated cluster, hot reload).
- **GitOps**: every DAG is a versioned, immutable artifact built in CI.
- A control plane you can actually **operate** — Go, observable, no GIL.

See the [architecture](/docs/architecture/) for how the pieces fit together.
