...
| `include_paths` | `["."]` | **Not implemented.** Declared and defaulted, but no build code reads it. What lands in the image is fixed by the Dockerfile, not by this field: the generated Dockerfile `COPY`s only `dag_source` (plus any dbt task-group project directories, or the whole build context when a group's `project` is `"."`); a project-supplied Dockerfile copies whatever its own `COPY` lines say. Tracked in [#1062](https://github.com/neochaotic/leoflow/issues/1062). |
